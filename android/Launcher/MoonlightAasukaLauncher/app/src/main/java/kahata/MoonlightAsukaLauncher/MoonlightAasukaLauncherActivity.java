// -*- mode: java -*-  Time-stamp: <2017-06-11 8:14:09 kahata>
/*================================================================
 * title: 愚鈍人ランチャー
 * file: MoonlightAasukaLauncherActivity.java
 * path: F:\GitHub\hataka\codingground\android\Launcher\MoonlightAasukaLauncher\app\src\main\java\kahata\MoonlightAsukaLauncher\MoonlightAasukaLauncherActivity.java
 * url:  http://192.168.1.53/f/GitHub/hataka/codingground/android/Launcher/MoonlightAasukaLauncher/app/src/main/java/kahata/MoonlightAsukaLauncher/MoonlightAasukaLauncherActivity.java
...
package kahata.MoonlightAsukaLauncher;
 ...
 *
 */
public class MoonlightAasukaLauncherActivity extends Activity implements OnItemClickListener, OnItemLongClickListener {
	...
	private Handler mHandler = new Handler(){
		public void handleMessage(android.os.Message msg) {
			...
			case ONCREATE:
				//getAppList(0);
				//getAppList(R.id.menu_aska_all);
				getAppList(menuindex);
				break;
			case ONITEMCLICK:
	...
	*/
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		myclasspath = new MoonlightAasukaLauncherActivity().getClass().getName();
		myclassname = new MoonlightAasukaLauncherActivity().getClass().getSimpleName();
	...
	private void getAppList(int menuid) {
		...
		for (ResolveInfo info : list) {
			...
			case R.id.menu_launcher:
				...
				break;
	 		case R.id.menu_aska_all:
				if(pkgname.indexOf("com.moonlight_aska")>-1) addAppInfo(info);
				break;
	 		case R.id.menu_aska_activity:
				if(pkgname.indexOf("com.moonlight_aska.activity")>-1) addAppInfo(info);
				break;

	 		case R.id.menu_aska_intent:
				if(pkgname.indexOf("com.moonlight_aska.intent")>-1) addAppInfo(info);
				break;

	 		case R.id.menu_aska_widget:
				if(pkgname.indexOf("com.moonlight_aska.widget")>-1) addAppInfo(info);
				break;

	 		case R.id.menu_aska_graphic:
				if(pkgname.indexOf("com.moonlight_aska.graphic")>-1) addAppInfo(info);
				break;
				
	 		case R.id.menu_aska_layout:
				if(pkgname.indexOf("com.moonlight_aska.layout")>-1) addAppInfo(info);
				break;

	 		case R.id.menu_aska_system:
				if(pkgname.indexOf("com.moonlight_aska.system")>-1) addAppInfo(info);
				break;

	 		case R.id.menu_aska_hardware:
				if(pkgname.indexOf("com.moonlight_aska.hardware")>-1) addAppInfo(info);
				break;

	 		case R.id.menu_aska_keytouch:
				if(pkgname.indexOf("com.moonlight_aska.keytouch")>-1) addAppInfo(info);
				break;
			case R.id.menu_aska_misc:
	 			special = pkgname.indexOf("com.moonlight_aska")>-1 &&
	 				pkgname.indexOf("com.moonlight_aska.activity")<0
					&& pkgname.indexOf("com.moonlight_aska.intent")<0
					&& pkgname.indexOf("com.moonlight_aska.widget")<0
						&& pkgname.indexOf("com.moonlight_aska.graphic")<0
						&& pkgname.indexOf("com.moonlight_aska.layout")<0
						&& pkgname.indexOf("com.moonlight_aska.system")<0
						&& pkgname.indexOf("com.moonlight_aska.hardware")<0
						&& pkgname.indexOf("com.moonlight_aska.keytouch")<0;
	 			if(special) addAppInfo(info);
	 			break;
	...
	private void startActivitySafe(int position) {
		...
		try {
			...
			new AlertDialog.Builder(MoonlightAasukaLauncherActivity.this)
			.setTitle("MyLauncher")
			.setCancelable(true)
				...
				public void onClick(DialogInterface dialog, int id) {
					...
					case 1:
						...
						.setItems(items, new DialogInterface.OnClickListener() {
							public void onClick(DialogInterface dialog, int item) {
								Toast.makeText(MoonlightAasukaLauncherActivity.this, 
			...
			if(pkgs.length ==1){
				try {
				  ...
				  while((len=input.read(buf))!=-1){
					...
					CommonLibrary.showDialog(MoonlightAasukaLauncherActivity.this,"error",e.toString());
					return false;
				}
			}
		}
...
///////////////////////////////////////////////////////////////////////////////
	private int menuindex = R.id.menu_aska_all;
	...
	@Override
		...
		case R.id.menu_website:
			uri = Uri.parse("http://wiki.livedoor.jp/moonlight_aska/");
			intent = new Intent(Intent.ACTION_VIEW,uri);
			startActivity(intent);
			return true;
