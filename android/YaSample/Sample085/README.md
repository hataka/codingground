# Lesson8 アプリの連携 データの共有
## Sample085 連絡帳の利用 p.225
Androidでは連絡先や画像・音楽・動画ファイルなど、端末に保存された各種のデータを共有しています。そこでこの節では、こうしたデータを利用してみます。  
ここでは連絡帳(コンタクトリスト)のデータを利用するアプロを作成します。連絡帳を利用するには、設定ファイルにパーミッションの設定が必要です。  
