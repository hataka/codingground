 ...
 *
 */
public class YaLauncherActivity extends Activity implements OnItemClickListener, OnItemLongClickListener {
	...
	public final static String WORKSPACE_LOCATION = "http://192.168.1.53/f/android/YASample/";
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		myclasspath = new YaLauncherActivity().getClass().getName();
		myclassname = new YaLauncherActivity().getClass().getSimpleName();
		...
		for (ResolveInfo info : list) {
			...
			switch(menuid){
				...
				case R.id.menu_launcher:
					if(pkgname.indexOf("Launcher")>-1)addAppInfo(info);
					break;
				case R.id.menu_ya_all:
					if(pkgname.indexOf("ya.")>-1) addAppInfo(info);
					break;
				case R.id.menu_lesson01:
					special = pkgname.indexOf("ya.Sample01")>-1
						|| pkgname.indexOf("ya.Rensyu01")>-1;
					if(special)addAppInfo(info);
					break;
				case R.id.menu_lesson02:
					special = pkgname.indexOf("ya.Sample02")>-1
						|| pkgname.indexOf("ya.Rensyu02")>-1;
					if(special)addAppInfo(info);
					break;
				case R.id.menu_lesson03:
    			special = pkgname.indexOf("ya.Sample03")>-1
    			|| pkgname.indexOf("ya.Rensyu03")>-1;
    			if(special)addAppInfo(info);
    			break;
    		case R.id.menu_lesson04:
    			special = pkgname.indexOf("ya.Sample04")>-1
    			|| pkgname.indexOf("ya.Rensyu04")>-1;
    			if(special)addAppInfo(info);
    			break;
    		case R.id.menu_lesson05:
    			special = pkgname.indexOf("ya.Sample05")>-1
    			|| pkgname.indexOf("ya.Rensyu05")>-1;
    			if(special)addAppInfo(info);
    			break;
    		case R.id.menu_lesson06:
    			special = pkgname.indexOf("ya.Sample06")>-1
    			|| pkgname.indexOf("ya.Rensyu06")>-1;
    			if(special)addAppInfo(info);
    			break;
    		case R.id.menu_lesson07:
    			special = pkgname.indexOf("ya.Sample07")>-1
    			|| pkgname.indexOf("ya.Rensyu07")>-1;
    			if(special)addAppInfo(info);
    			break;
    		case R.id.menu_lesson08:
    			special = pkgname.indexOf("ya.Sample08")>-1
    			|| pkgname.indexOf("ya.Rensyu08")>-1;
    			if(special)addAppInfo(info);
    			break;
    		case R.id.menu_lesson09:
    			special = pkgname.indexOf("ya.Sample09")>-1
    			|| pkgname.indexOf("ya.Rensyu09")>-1;
    			if(special)addAppInfo(info);
    			break;
    		case R.id.menu_lesson10:
    			special = pkgname.indexOf("ya.Sample10")>-1
    			|| pkgname.indexOf("ya.Rensyu10")>-1;
    			if(special)addAppInfo(info);
    			break;
    		case R.id.menu_lesson11:
    			special = pkgname.indexOf("ya.Sample11")>-1
    			|| pkgname.indexOf("ya.Rensyu11")>-1;
    			if(special)addAppInfo(info);
    			break;
    		case R.id.menu_lesson12:
    			special = pkgname.indexOf("ya.Sample12")>-1
    			|| pkgname.indexOf("ya.Rensyu12")>-1;
    			if(special)addAppInfo(info);
    			break;
	...
	private void startActivitySafe(int position) {
		...
		try {
			...
			new AlertDialog.Builder(YaLauncherActivity.this)
			.setTitle("MyLauncher")
			.setCancelable(true)
			...
			.setPositiveButton("Ok", new DialogInterface.OnClickListener() {
				public void onClick(DialogInterface dialog, int id) {
					...
					case 1:
						...
						.setItems(items, new DialogInterface.OnClickListener() {
							public void onClick(DialogInterface dialog, int item) {
								Toast.makeText(YaLauncherActivity.this, 
				...
				*/
						...
						//intent.putExtra("keyword", lines.toString());
						intent2.setClassName("kahata.GudonLauncher", "kahata.GudonLauncher.SubActivity");
			...
			if(pkgs.length ==1){
				try {
				  ...
				  while((len=input.read(buf))!=-1){
					...
					CommonLibrary.showDialog(YaLauncherActivity.this,"error",e.toString());
					return false;
				}
			}
		}
...
///////////////////////////////////////////////////////////////////////////////
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		case R.id.menu_website:
			uri = Uri.parse("http://homepage3.nifty.com/~mana/");
			intent = new Intent(Intent.ACTION_VIEW,uri);
			startActivity(intent);
			return true;
		...
		case R.id.menu_pukiwiki2012:
			uri = Uri.parse("http://192.168.24.92/pukiwiki2012/?Android%E9%96%8B%E7%99%BA%2FYASample");
			intent = new Intent(Intent.ACTION_VIEW,uri);
			startActivity(intent);
			return true;
